...
function tokenize(src) {
  ...
  while (i < n) {
    ...
    if (/[0-9]/.test(c) || (c === '.' && /[0-9]/.test(src[i + 1]))) {
      ...
      push('num', Number(src.slice(i, j))); i = j; continue;
    }
    if (/[A-Za-z_]/.test(c)) {
...
class Parser {
  ...
  // program := 'function' type? IDENT '(' ')' block   (we accept just a bare block too)
  parseProgram() {
    if (this.is('id', 'function')) {
      this.next();
      if (this.is('id') && TYPE_KEYWORDS.has(this.peek().v)) this.next(); // return type
      if (this.is('id')) this.next(); // function name (Eval)
...
function swizzle(v, s) {
  if (![...s].every((ch) => ch in SW)) return null; // not a swizzle (e.g. a method we missed)
  return [...s].map((ch) => v[SW[ch]] ?? 0);
}
...
//   scene{...}, rand(a,b), vrand(a,b), kill(), trigger(name)
export function compileScript(src) {
  const ast = new Parser(tokenize(src)).parseProgram();
  return { ast, run: (ctx) => execBlock(ast, ctx, new Map()) };
}

function execBlock(block, ctx, locals) {
  for (const s of block.stmts) execStmt(s, ctx, locals);
}
function execStmt(s, ctx, locals) {
  switch (s.k) {
    case 'block': return execBlock(s, ctx, new Map(locals));
    case 'empty': return;
    case 'decl': { const v = s.init ? evalExpr(s.init, ctx, locals) : [0]; locals.set(s.name, v.slice()); return; }
    case 'exprstmt': evalExpr(s.expr, ctx, locals); return;
    case 'assign': return execAssign(s, ctx, locals);
  }
}
function execAssign(s, ctx, locals) {
  const rhs = evalExpr(s.rhs, ctx, locals);
  // target: plain id, or member (single-component swizzle write) on an id
  let name, comp = null;
  if (s.lhs.k === 'id') name = s.lhs.name;
  else if (s.lhs.k === 'member' && s.lhs.obj.k === 'id') { name = s.lhs.obj.name; comp = s.lhs.member; }
  else throw new Error('script: unsupported assignment target');

  const cur = locals.has(name) ? locals.get(name) : (ctx.getField(name) ?? [0]);
  let next;
  if (comp) {
    next = cur.slice();
    const idx = SW[comp[0]] ?? 0;
    const cv = applyCompound(s.op, [cur[idx] ?? 0], rhs);
    next[idx] = cv[0];
  } else {
...
function evalExpr(e, ctx, locals) {
  switch (e.k) {
    case 'num': return [e.v];
    case 'neg': return vneg(evalExpr(e.e, ctx, locals));
    case 'not': return [evalExpr(e.e, ctx, locals)[0] ? 0 : 1];
    case 'id': return evalId(e.name, ctx, locals);
    case 'bin': return evalBin(e, ctx, locals);
    case 'cond': return evalExpr(e.c, ctx, locals)[0] ? evalExpr(e.a, ctx, locals) : evalExpr(e.b, ctx, locals);
    case 'member': return evalMember(e, ctx, locals);
    case 'method': return evalMethod(e, ctx, locals);
    case 'call': return evalCall(e, ctx, locals);
  }
...
function evalBin(e, ctx, locals) {
  const a = evalExpr(e.left, ctx, locals), b = evalExpr(e.right, ctx, locals);
  switch (e.op) {
...
function evalCall(e, ctx, locals) {
  ...
  if (fn) return fn(...args);
  throw new Error('script: unknown function ' + e.name);
}
...
const INTRINSICS = {
  ...
  bias: (x, e) => x.map((v) => Math.pow(v, Math.pow(2, -e[0]))),
