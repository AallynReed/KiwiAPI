...
CODEX_PARSER_VERSION = 25  # v25: a MultiplySum record gets the `…Bonus` twin of its stat key ($Stat_PhysicalDamageBonus -> "Physical Damage Bonus") - the operation byte, not the stat id, decides what a record means, and MultiplySum SCALES the stat where Add adds to it. The two were one name, which is invisibly wrong wherever both display as a percentage: +30% Critical Damage is 30 points under Add and 900 under MultiplySum on a 3,000% sheet. Also completes the operation table from the game's own KModType enum (Nullify/Minimum/Maximum were falling through to Add)
