...
CODEX_PARSER_VERSION = 27  # v27: a dragon's unlocked-forever bonuses open under their own `3e AE` marker and name no slot; unrecognised, the block inherited the last slot still open (always wings), so every dragon's "+1000 Max Health / +Light / +Magic Find" was stored as a Wings bonus - 234 rows across 67 collections
# v26: the `…Bonus` split generalized from crit damage to EVERY stat - v25 shipped it for stat id 0x16 only and then kept the same version number when the rule widened, so the branch had already rebuilt at 25 and never re-parsed. Any MultiplySum record now carries the `…Bonus` twin of its key (551 collection records across 14 keys, not 12)
# v25: MultiplySum SCALES the stat where Add adds to it, so the two stopped sharing one key and one name; also completed the operation table from the game's own KModType enum
# v24: ability names and descriptions come from the referenced ability prefab instead of being derived from the ref - the derived description key had the wrong shape for part of the archive, and the derived NAME was an internal id in title case ("Enemydeath Damagebuff"), shown where the game names nothing at all
