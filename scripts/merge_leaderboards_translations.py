...
TRANSLATIONS: dict[str, dict[str, str]] = {
    ...
    "fr": {
        ...
        "Captures come from an in-game scraping bot every hour. Boards reset on the same cadence Trove does (daily 11:00 UTC; weekly contests roll Tuesdays).": "Les captures proviennent d'un bot dans le jeu, chaque heure. Les classements se réinitialisent à la même cadence que Trove (quotidien 11:00 UTC ; concours hebdomadaires le mardi).",
    ...
    "de": {
        ...
        "Captures come from an in-game scraping bot every hour. Boards reset on the same cadence Trove does (daily 11:00 UTC; weekly contests roll Tuesdays).": "Erfassungen kommen stündlich von einem In-Game-Scraping-Bot. Listen werden im selben Rhythmus zurückgesetzt wie Trove (täglich 11:00 UTC; wöchentliche Wettbewerbe dienstags).",
    ...
    "pt-PT": {
        ...
        "Captures come from an in-game scraping bot every hour. Boards reset on the same cadence Trove does (daily 11:00 UTC; weekly contests roll Tuesdays).": "As capturas vêm de um bot dentro do jogo, a cada hora. As classificações reiniciam à mesma cadência do Trove (diariamente às 11:00 UTC; concursos semanais rolam às terças-feiras).",
    ...
    "ru": {
        ...
        "Captures come from an in-game scraping bot every hour. Boards reset on the same cadence Trove does (daily 11:00 UTC; weekly contests roll Tuesdays).": "Снимки делает игровой скрейпинг-бот каждый час. Таблицы сбрасываются в том же ритме, что и в Trove (ежедневно в 11:00 UTC; еженедельные конкурсы — по вторникам).",
    ...
    "ja": {
        ...
        "Load more": "もっと読み込む",
        "Captures come from an in-game scraping bot every hour. Boards reset on the same cadence Trove does (daily 11:00 UTC; weekly contests roll Tuesdays).": "キャプチャはゲーム内のスクレイピングボットから毎時取得されます。ボードは Trove と同じサイクルでリセットされます（毎日 11:00 UTC、週次コンテストは火曜日）。",
    ...
    "zh-CN": {
        ...
        "Load more": "加载更多",
        "Captures come from an in-game scraping bot every hour. Boards reset on the same cadence Trove does (daily 11:00 UTC; weekly contests roll Tuesdays).": "采集由游戏内抓取机器人每小时执行。榜单按 Trove 自身的节奏重置（每日 11:00 UTC；每周比赛在周二轮换）。",
